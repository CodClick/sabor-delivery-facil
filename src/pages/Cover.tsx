...
const Cover: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      <div className="w-full max-w-[600px] mx-auto px-4 pt-4 space-y-6">
        {/* Banner 1 - 600x400 (3:2) */}
        {settings.cover_banner1_url && (
          <div className="w-full aspect-[3/2] overflow-hidden rounded-lg shadow-md">
            <img
        ...
        <div className="grid grid-cols-3 gap-3 sm:gap-4">
          {buttons.map(({ label, icon: Icon, onClick }) => (
            <button
              key={label}
              onClick={onClick}
              className="flex flex-col items-center justify-center gap-2 aspect-square rounded-2xl shadow-md transition-transform active:scale-95 hover:opacity-90"
              style={{ backgroundColor: settings.cover_botao_cor_fundo || settings.cor_primaria }}
            >
              <Icon className="h-8 w-8 sm:h-10 sm:w-10" strokeWidth={1.75} style={{ color: settings.cover_botao_cor_icone || '#ffffff' }} />
              <span

...
interface AppliedCoupon {
  ...
  // Escolha do cliente quando produto_brinde.modo === "escolha"
...
  };
  ...
  // Resolve qual será o brinde efetivo (suporta modo "escolha")

  const resolverBrindeEfetivo = (
    coupon: AppliedCoupon
  ): { product_id: string; product_name: string; quantidade: number } | null => {
    const brinde = coupon.produto_brinde;
    if (!brinde) return null;
    if (brinde.modo === "escolha") {
      const escolha = coupon.brinde_escolhido;
      if (!escolha?.product_id) return null;
      // Validar que a escolha está entre as opções configuradas
      const opcoes = brinde.opcoes || [];
      const valida = opcoes.some((o) => o.product_id === escolha.product_id);
      if (!valida) return null;
      return {
        product_id: escolha.product_id,
        product_name: escolha.product_name,
        quantidade: brinde.quantidade || 1,
      };
    }
    return {
      product_id: brinde.product_id,
      product_name: brinde.product_name,
      quantidade: brinde.quantidade || 1,
    };
  };
  ...
  const buildGiftItem = (
    ...
    const efetivo = resolverBrindeEfetivo(coupon);
    if (!efetivo) return null;
    const ciclos = computeGiftMultiplier(items, coupon.produtos_requeridos || []);
    if (ciclos <= 0) return null;
    const totalBrindes = (efetivo.quantidade || 1) * ciclos;
    return {
      id: `gift-${efetivo.product_id}-${coupon.id}`,
      name: `🎁 ${efetivo.product_name} (Brinde)`,
      description: `Brinde do cupom ${coupon.nome}`,
      price: 0,
      image: "/placeholder.svg",
      category: "brinde",
      quantity: totalBrindes,
      [BRINDE_FLAG]: efetivo.product_id,
    } as any;
  };
  ...
  useEffect(() => {
    ...
    } else if (brindeAtual.id !== novoBrinde.id || brindeAtual.quantity !== novoBrinde.quantity) {
      setCartItems((prev) => [...prev.filter((i) => !isGiftItem(i)), novoBrinde]);
    }
  ...
  // Atualiza apenas a escolha do brinde no cupom já aplicado
  const setBrindeEscolhido = (
    escolha: { product_id: string; product_name: string } | null
  ) => {
    setAppliedCoupon((prev) =>
      prev ? { ...prev, brinde_escolhido: escolha } : prev
    );
  };
